...
const BlogRegistration = ({keyword, url, isBlog, isView, handleChangeInput, handleChangeOption, handleSubmit}) => {
  ...
  return (
    <div className={cx('blog-rs-form')}>
      ...
      <div className={cx('option-box')}>
        <div>블로그</div>
        <div className={cx('option-btn', 'blog-btn')} onClick={() => onChangeOption('isBlog')}>
          <div className={cx('move-btn', { 'selected': isBlog})}>
            {isBlog ? 'On' : 'Off'}
          </div>
        </div>
        <div>뷰</div>
        <div className={cx('option-btn', 'view-btn')} onClick={() => onChangeOption('isView')}>
